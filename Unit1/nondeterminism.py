...
def dfsmsim(string, current, edges, accepting):
    if string == "":
        ...
        if (current, letter) in edges:
            ...
            return dfsmsim(remaining_string, destination, edges, accepting)
        else:
            return False

# My first solution:
def nfsmsim_first(string, current, edges, accepting):
    if string == "":
        return current in accepting
    else:
        letter = string[0]
        if (current, letter) in edges:
            if len(edges[(current, letter)]) == 1:
                current = edges[(current, letter)][0]
            else:
                for dest in edges[(current, letter)]:
                    if (dest, string[1]) in edges:
                        current = dest
            string = string[1:]
            return nfsmsim(string, current, edges, accepting)
        else:
            return False       
...
# Peter's solution:
def nfsmsim_peter(string, current, edges, accepting):
    if string == "":
        return current in accepting
    else:
        letter = string[0:1]
        if (current, letter) in edges:
            remainder = string[1:]
            newstates = edges[(current, letter)] # this will be a list of the states
